...
function evaluateCurve(motionData: CubismMotionData, index: number, time: number): number {
  ...
  for (let i: number = curve.baseSegmentIndex; i < totalSegmentCount; ++i) {
    ...
    pointPosition =
      ...
      (motionData.segments.at(i).segmentType == CubismMotionSegmentType.CubismMotionSegmentType_Bezier ? 3 : 1);
...
export class CubismMotion extends ACubismMotion {
  ...
  public doUpdateParameters(
    ...
    for (
      c = 0;
      c < this._motionData.curveCount && curves.at(c).type == CubismMotionCurveTarget.CubismMotionCurveTarget_Model;
      ++c
    ) {
    ...
    for (
      ;
      c < this._motionData.curveCount && curves.at(c).type == CubismMotionCurveTarget.CubismMotionCurveTarget_Parameter;
      ++c
    ) {
