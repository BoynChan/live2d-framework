    ...
    if (usingClipCount > 0) {
      ...
      for (let clipIndex = 0; clipIndex < this._clippingContextListForMask.getSize(); clipIndex++) {
        ...
        if (renderer.isUsingHighPrecisionMask()) {
          ...
          const maskPixelSize: number = clipContext.getClippingManager()._clippingMaskBufferSize;
  ...
  public setupLayoutBounds(usingClipCount: number): void {
    ...
    if (usingClipCount <= 0 || usingClipCount > useClippingMaskMaxCount) {
      ...
      for (let index = 0; index < this._clippingContextListForMask.getSize(); index++) {
        const clipContext: CubismClippingContext = this._clippingContextListForMask.at(index);
...
export class CubismRenderer_WebGL extends CubismRenderer {
  ...
  public doDrawModel(): void {
    ...
    for (let i = 0; i < drawableCount; ++i) {
      ...
      const clipContext =
        this._clippingManager != null ? this._clippingManager.getClippingContextListForDraw().at(drawableIndex) : null;
